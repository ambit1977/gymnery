...
## 8. テスト・リグレッション防止体制
Vanilla JSで構築されているため、グローバル変数への依存や関数参照エラーによる重大なデグレ（画面が真っ白になる等）が発生しやすい構造です。
新規機能追加やUIレンダリング関数（renderStats等）の改修を行った際は、以下の手順を必ず実施してください。
1. 結合（スモーク）テストの記述: test/app.test.js 内に、修正したレンダリング関数を呼び出すテストを追加します。
2. 依存モックの整備: 内部で呼ばれる関数（APIアクセスやDBアクセス）は sandbox にモックを追加します。
3. npm test の必須実行: プッシュ前に必ず npm test（Vitest）を実行し、ReferenceErrorなどが発生せずHTML出力が最後まで完了することを確認します。
